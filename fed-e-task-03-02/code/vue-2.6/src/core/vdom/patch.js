...
export function createPatchFunction(backend) {
    ...
    function patchVnode(
        ...
        if (isUndef(vnode.text)) { // 新节点没有文本
            if (isDef(oldCh) && isDef(ch)) { // 新老节点的子节点都存在
                // 如果老节点的子节点和新节点的子节点不相等，调用 updateChildren
                if (oldCh !== ch) updateChildren(elm, oldCh, ch, insertedVnodeQueue, removeOnly)
            } else if (isDef(ch)) { // 新节点有子节点，而老节点没有子节点
                if (process.env.NODE_ENV !== 'production') {
        ...
        } else if (oldVnode.text !== vnode.text) { // 新老节点的文本不同
            // 修改文本
    ...
    // 函数柯里化，让一个函数返回一个函数
    // createPathFunction({nodeOPs, modules}) 传入平台相关的两个参数

    // core 中的 createPathFunction(backend), const {modules,nodeOps} = backend
